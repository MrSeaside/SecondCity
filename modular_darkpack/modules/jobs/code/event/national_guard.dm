...
/datum/antagonist/national_guard/proc/offer_loadout()
	...
	switch(loadout_type)
		...
		if("EOD")
			owner.current.put_in_r_hand(new /obj/item/clothing/suit/darkpack/eod(owner.current))
			owner.current.put_in_l_hand(new /obj/item/clothing/head/darkpack/eod(owner.current))
		if("Medic")
